...
macOS-optimized with automatic architecture detection.
"""
...
class PiperTTS:
    """Piper TTS wrapper with macOS support and auto architecture detection."""
    ...
    def _detect_platform(self):
        ...
        if system == "Darwin":  # macOS
            if machine == "arm64":  # Apple Silicon (M1, M2, M3, etc.)
                self.piper_url = f"https://github.com/rhasspy/piper/releases/download/{self.PIPER_VERSION}/piper_macos_aarch64.tar.gz"
                self.piper_dir_name = "piper_macos_aarch64"
            elif machine == "x86_64":  # Intel Mac
                self.piper_url = f"https://github.com/rhasspy/piper/releases/download/{self.PIPER_VERSION}/piper_macos_x86_64.tar.gz"
                self.piper_dir_name = "piper_macos_x86_64"
            else:
                print(f"{YELLOW}[TTS] Unknown macOS architecture: {machine}{RESET}")
                self.available = False
                return
            
            self.piper_exe_name = "piper"
            print(f"{CYAN}[TTS] Detected macOS {machine} architecture{RESET}")
        elif system == "Windows":
            self.piper_url = f"https://github.com/rhasspy/piper/releases/download/{self.PIPER_VERSION}/piper_windows_amd64.zip"
            self.piper_dir_name = "piper_windows"
            self.piper_exe_name = "piper.exe"
            print(f"{CYAN}[TTS] Detected Windows architecture{RESET}")
        elif system == "Linux":
            if machine == "x86_64":
                self.piper_url = f"https://github.com/rhasspy/piper/releases/download/{self.PIPER_VERSION}/piper_linux_x86_64.tar.gz"
                self.piper_dir_name = "piper_linux_x86_64"
            else:
                print(f"{YELLOW}[TTS] Unsupported Linux architecture: {machine}{RESET}")
                self.available = False
                return
            
            self.piper_exe_name = "piper"
            print(f"{CYAN}[TTS] Detected Linux {machine} architecture{RESET}")
        else:
            print(f"{YELLOW}[TTS] Unsupported OS: {system}{RESET}")
            self.available = False
    
    def _download_piper_executable(self):
        """Download and extract Piper executable for the current platform."""
        if not self.available:
            return None
        
        piper_exe_dir = self.piper_dir / self.piper_dir_name
        piper_exe = piper_exe_dir / self.piper_exe_name
        ...
        try:
            r = http_session.get(self.piper_url, stream=True)
            r.raise_for_status()
            ...
            if self.piper_url.endswith('.tar.gz'):
                # Extract tar.gz
                with tarfile.open(fileobj=file_data, mode='r:gz') as tf:
                    piper_exe_dir.mkdir(parents=True, exist_ok=True)
                    for member in tf.getmembers():
                        if member.name.endswith(f'/{self.piper_exe_name}') or member.name.endswith(self.piper_exe_name):
                            # Extract just the executable
                            target_path = piper_exe_dir / self.piper_exe_name
